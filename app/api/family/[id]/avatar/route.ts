...
import {
  clearFamilyMemberAvatar,
  generateFamilyMemberAvatar,
  saveFamilyMemberAvatarUpload,
} from "@/lib/family/avatar";
import {
  getFamilyMemberById,
...
export async function POST(request: Request, context: Ctx) {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      await saveFamilyMemberAvatarUpload(id, buf);
      return NextResponse.json({
        ok: true,
        member: getFamilyMemberPublic(id),
      });
    }
    ...
    if (parsed.data.gender !== undefined) {
      updateFamilyMember(id, { gender: parsed.data.gender });
    }
    await generateFamilyMemberAvatar(id, {
