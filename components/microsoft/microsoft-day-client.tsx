...
export function MicrosoftDayClient() {
  ...
  const loadMail = useCallback(async (from?: string, to?: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/microsoft/mail/today?${qs}`);
      const json = await res.json();
  ...
  function slotDurationFor(e: MsEvent): number {
    return (
  ...
  async function suggestSlots(eventId: string, durationMinutes?: number) {
    const event = events.find((x) => x.id === eventId);
    const duration =
      durationMinutes ??
      (event
        ? slotDurationFor(event)
        : 60);
    setSlotDurationByEvent((prev) => ({ ...prev, [eventId]: duration }));
    setBusyId(eventId);
    setError(null);
    try {
      const res = await fetch("/api/microsoft/calendar/actions", {
  ...
  const pollJobOnce = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/microsoft/mail/analyze?${qs}`);
      const json = await res.json();
      ...
      {connected ? (
        <>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-[var(--brand-docs)]">
              Verbunden als{" "}
