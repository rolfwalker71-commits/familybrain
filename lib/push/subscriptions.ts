...
export function deletePushSubscriptionById(
  ...
  const db = getDb();
  const info = db
    .prepare(`DELETE FROM push_subscriptions WHERE owner_key = ? AND id = ?`)
    .run(ownerKey, id);
  return info.changes > 0;
}

export function listPushSubscriptionsForOwner(
  ownerKey: string
): PushSubscriptionRow[] {
  const db = getDb();
  return db
    .prepare(
      `SELECT * FROM push_subscriptions WHERE owner_key = ? ORDER BY id DESC`
    )
    .all(ownerKey) as PushSubscriptionRow[];
}
