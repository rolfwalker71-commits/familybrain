...
function ownerMayReceive(
  ...
  const domain = NOTIFY_REASON_DOMAIN[notification.reason] || notification.domain;
  if (domain === "documents") {
  ...
  // Scoped travel/finance without id: allow if user has any access (prefs refine)
  if (domain === "travel") {
    return true;
  }
  if (domain === "finance") {
...
/** Fan-out web push to subscribers whose prefs allow this notification. */
