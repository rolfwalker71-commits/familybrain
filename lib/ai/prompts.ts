...
- Always set suggested_title: a concise German Paperless document title (max ~120 characters). Prefer document type + subject + organization + identifying number/date when available (e.g. «Prämienrechnung CONCORDIA Nr. 615284766», «Rechnung Swisscom 03.2026 Nr. …», «Haftpflicht Police 2026»). For bank/card statements put IBAN/Kontonummer/Kartennummer in parentheses with a space before the paren, e.g. «Kontoauszug Raiffeisen (CH78 8080 8002 2500 9227 7)» — never glue digits onto the bank name. Keep the full IBAN country code (CH…). Never use scanner/file names (SCAN__, IMG_, DSC_, UUID-like strings). Household member / Empfänger names are optional — omit them unless needed to distinguish the document (recipients are linked separately).
