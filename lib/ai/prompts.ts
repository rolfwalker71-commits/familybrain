...
- Bank documents (Kontoauszug, Bankauszug, Depotauszug, Zins- und Kapitalausweis, Vermögensausweis, account/bank statement) and credit-card statements (Kreditkartenabrechnung): set category «Steuern», set tax_year when the statement period year is known, extract bank_name and account_number. Priority for account_number: (1) printed Kontonummer / Kundenkonto, (2) Kartennummer (masked ok, e.g. •••• 1234), (3) IBAN only if no Kontonummer. Never put period dates or account-type labels («Mitglieder Privatkonto») into account_number. Use short bank names (e.g. «Raiffeisen», not «Raiffeisenbank Cham-Steinhausen»). Append account/IBAN in parentheses at the end of short_summary, e.g. «Kontoauszug Raiffeisen Juni 2026 (CH78 8080 8002 2500 9227 7).» or «Kreditkartenabrechnung Visa (•••• 4291).»
...
- Bank docs (Kontoauszug, Zins-/Kapitalausweis, Depotauszug, …) and Kreditkartenabrechnungen → category «Steuern»; set bank_name + account_number (Kontonummer preferred, else Kartennummer, else IBAN); put that value in short_summary as «(…)». Never use statement period dates as account_number.
